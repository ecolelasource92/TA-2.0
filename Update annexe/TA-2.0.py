...
def menu1(eleves, matiere) : #on définis la fonction qui sera executer lorsque l'utilisateur est un élève
  ...
  return(eleves) # retourner les valeurs de la focntion
...
def menu2(profs, eleves) : #on définis la fonction à executer si l'utilisateur est un professeur
  ...
  connexion = input("Entrer votre nom d'utilisateur: ") #entrer le nom du professeur
            ...
            if password == prof[1]: #verifier le mot de passe
                   ...
                   for eleve in eleves : #boucle pour le nombre d'eleve 
                          ...
                          if eleveVeto == eleve[0]:  #si l'eleve à placer existe
                                 ...
                                 eleve[7] = 1 #on enrengistre le fait que l'élève est prioritaire dans se TA dans sa variable Veto en lui donnant la valeur 1
...
#création d'une boucle itérative (iterateur : nombre d'élève (liste "eleves"))
     for loop in range(len(eleves)):
       ...
       if m == 1 and eleves[loop][7] == 0 :     #on ajoute la justice correspondant au nombre de tour effectué
...
#on demande les 4 matières 1 par une
...
def ajoutereleve(eleves): #fonction d'ajout d'élève dans la liste des eleves (accès admin)
...
#algorythme d'affichage du menu + déclenchement des fonction associées
